...
## Reglas de validación consolidadas — v16
...
**5.2 Los tres avances.** Ninguno cuenta filas conformes: los tres miden
**criterios cumplidos**.
...
Avance general      =  Σ criterios de productos + Σ criterios de procesos
del Anexo 1            ──────────────────────────────────────────────────
                        (nº productos × 8) + (nº procesos × 5)
```
...
denominación en mayúsculas (regla 9).

En el bloque de procesos entran los de Nivel 0, los subprocesos y las
catalogaciones. Quedan fuera los `NO APLICA` —PE.03 y PS.08 ausentes—, que no se
puntúan ni cuentan en el denominador.
...
**Por qué el general no es el promedio de los otros dos.** Promediarlos daría el
mismo peso a los 206 productos de la FDCP que a sus 81 procesos. Sumando
criterios, cada bloque pesa lo que hubo que revisar en él. Ejemplo: 10 productos
al 100 % (80 de 80 criterios) y 1 proceso al 0 % (0 de 5) dan **94 %**; el
promedio simple daría 50 %.
...
## 12. Fila de totales del resumen — *NUEVA en v15*
...
criterios evaluados**, con la misma fórmula de la regla 5.2 aplicada al conjunto:

```
Avance total  =  Σ criterios cumplidos de las 20 facultades
                 ───────────────────────────────────────────
                 Σ criterios que había que cumplir en las 20
```

**No es el promedio de los 20 porcentajes**, y la diferencia no es menor.
Promediarlos daría el mismo peso a la FII, con 30 productos, que a la FO, con
283: una facultad pequeña y floja hundiría el total tanto como una grande.
Ponderando, cada facultad pesa lo que aportó al trabajo de revisión.

Sobre los datos de la última corrida, el ponderado da **84 %** y el promedio
simple **79 %**: cinco puntos de diferencia, en contra de las facultades
grandes que sí llenaron.

Los avances se calculan desde los **criterios crudos** de cada facultad, no
desde sus porcentajes ya redondeados, de modo que el total no arrastra veinte
redondeos.